...
18. **The `you` family removed entirely; the pronoun system stated** (owner directive 2026-07-26).
   ...
   - **Open, not yet ruled — the absolute possessive in the modern edition.** `MODERN_RULES` in `scripts/custom_export.py` is word-for-word and maps `thine → your` in every position, so the **37 verses** where *thine* is the absolute possessive read *your* where Present-Day English needs ***yours***: Luke 22:42 "not my will, but **your**, be done"; I Chr 29:11 "**Your**, O Lord, is the greatness"; I Cor 3:21 "all things are **your**"; John 17:10; Matt 6:13. The original-voice edition is unaffected. A fix needs a context-sensitive engine rule (*thine* not followed by a word it modifies → *yours*, the same absolute test `scripts/83` already uses); a settings `GlobalReplacements` entry maps word-to-word and cannot make the distinction.
