...
- [x] Date/validate the uncleared word list — **done via king-james agent review (owner directive 2026-07-14)**: 8 parallel first-pass batches over all 2,060 words, then a verse-verified second-opinion pass over the 65 first-pass flags. **Result: all 2,060 words judged genuine 1611 vocabulary** — the uncleared list was corpus-coverage gaps (rare -eth/-est forms, Hebrew loanwords, KJV compounds like "mercy seatward", possessive tokens like "for forty's sake"), not corruption. Verdicts + notes in `word_era` (source `KJ-agent 2026-07-14`); TSVs in `references/word_reviews/`; report `references/word_review_report.md`. The first pass's 10 "strong belief" alternates were all overturned by direct verse lookup (e.g. "intreaty" is the KJV's own spelling; "leaved" is Isaiah 45:1 "two leaved gates").
