...
24. **Second archaism pass — Claude's own review** (owner directive 2026-07-27: *"review also from your own perspective words and phrases that should be modernized and apply them to the settings file"*).
   - Method differed from #23 deliberately. The King James agent worked *down* a frequency list of surviving word types; this pass worked *outward* from a candidate list of KJV-era vocabulary and idiom built from knowledge of the register — which is what surfaced the **phrases**, a whole category the first pass had almost entirely missed (a word-frequency list cannot see *by and by* or *take no thought*). Each candidate was counted in both the 1611-voice input and the modern output, so anything the built-in pass already handled was dropped immediately (that is how `charity`, `twain`, `peradventure`, `naught` were ruled out — already converted).
   - **74 further rules, bringing `custom/site-modern.json` to 119.**
     - **Phrases (24)** — the category the first pass missed: `to wit`→namely (21), `of a truth`→truly (18), `after this manner`→in this way (16), `in no wise`→in no way (12), `must needs`→must (12), `gave up`/`yielded up the ghost`→died (13), `on this wise`→in this way (7), `suffer me`→allow me (7), `in ward`→in custody (7), `by and by`→immediately (4, a false friend — it means *at once*, the opposite of today's "eventually"), and the five `take no thought` word-orders → *do not worry* (Matt 6:25-34, Luke 12:11).
     - **False friends (Claude's finds)** — `dumb`→**mute** (32; today's dominant sense is an insult), `careful for nothing`→**anxious for nothing** (Phil 4:6), `careful and troubled`→anxious and troubled (Luke 10:41), `carefulness`→anxiety (3), `lively`→**living** in its three fixed phrases (*lively stones*, *lively hope*, *lively oracles*), `gin`/`gins`→**trap**/traps (5 — not the drink), `seethe`/`seething`→**boil**/boiling (12 — not anger), `ward`→custody, `nought`→nothing (37).
     - **Plain archaisms** — `raiment`→clothing (59), `damsel`(s/'s)→young woman/women (53), `husbandman/men`→farmer/s (35), `kine`→cows (24), `rend`→tear (20), `thrice`→three times (15), `nether`→lower (15), `intreat`(ed)→entreat(ed) (33), `requite`/`requited`/`requiting`→repay/repaid/repaying (12), `forthwith`→immediately (11), `chide`/`chiding`→quarrel/quarreling (5), `scrip`→bag (8), `wont`→accustomed (8), `haply`→perhaps (6), `athirst`→thirsty (5), `lucre`→gain (6), `succour`(ed)→help(ed) (5), `ofttimes`→often (3), `twoedged`→two-edged (3).
   - **`wax` needed 13 phrase rules rather than one word rule.** The verb sense (grow) and the literal substance share a spelling: *"my heart is like wax; it is melted"* (Ps 22:14), *"as wax melteth before the fire"* (Ps 68:2). So `waxed`→grew, `waxen`→grown, `waxeth`→grows, `waxing`→growing are safe as words, but the bare present tense ships as collocations only — `wax hot`, `wax cold` (Matt 24:12), `wax old`, `wax rich`, `wax poor`, `wax dim`, `wax warm`, `wax lean`, `wax pale`, `wax feeble`, `wax wanton`, `wax worse`, and `wax not old` ahead of `wax old`. **Verified: the 4 literal *wax* survive untouched.**
   - **Rejected, with the reason** — `careful`/`carefully` as blanket rules (Titus 3:8 *"careful to maintain good works"* means diligent, where "anxious" inverts it — only the two anxiety phrases ship); `reins` (15 — a true false friend, today read as horse tack, but Job 16:13 and Lam 3:13 are physically the kidneys while the rest are figurative, so no one word serves); `bowels` (40 — same split between anatomy and *bowels of mercies*); `wherefore` (348 — needs *why* interrogative vs *therefore* conjunctive, two senses one spelling); `rent` (67 — past tense of *rend*, but also the noun *the rent is made worse*); `withal` (33 — means "with which", and every one-word substitute reads as redundant); `comely` (19 — splits between *handsome* and *fitting*); `it came to pass` (453 — understandable, and rewriting it would remake the book's cadence; an owner call, not mine); plus `cleave`, `countenance`, `mirth`, `strife`, `travail`, `whoredom`, `firmament`, `cockatrice` (creature name), and the measures (*omer*, *ephah*, *hin*) as terms of art.
   - Verified: every targeted word and phrase is gone from the modern body, the 1611-voice edition is untouched (still 59 *raiment*, 49 *damsel*, 37 *waxed*), and the re-run is byte-identical. One methodological note recorded for future passes — an unbounded `"to wit"` search reports false positives on *to wit**ness***; the engine's own keys carry `\b` boundaries and are not affected.
...
- [x] **EXPANDED 2026-07-26/27, again 2026-07-29** (owner directive) — the whitelist grew from **692 to 4,462 reviewed words** in four steps, all folded into the owner-reviewed source `rare_word_review_no_safe_swap.md` so `29_build_whitelist.py` reproduces them:
  ...
  4. **Owner's manual token-list pass, 2026-07-29** — `scripts/92_whitelist_tokenlist_manual.py` folded in the **2,023 word forms** the owner hand-picked out of `references/word_lists/token_list_full.md` and listed in `references/word_lists/tokenlist_manual_whitelist.md` (built up over the commits of 2026-07-28/29). The selection covers **1,094 complete inflection groups with no partially-covered group** — every form of each chosen group is listed — and every listed word verified as a real token form of the restored text. Entries carry the `tokenlist-manual` tag and an `OWNER RULING 2026-07-29: DO NOT CHANGE` line; the script is idempotent (verified no-op on re-run). Whitelist **2,439 → 4,462 reviewed** + 3,602 proper nouns. **26 of the 2,023 were on the blacklist**, having been changed in one or more verses by an earlier pass (abodest, acceptest, accepting, accompany, accompanying, babbling, babblings, channel, closer, divorce, engrave, fewer, firebrand, fornicator, lock, locked, pot, reaper, reprover, road, sharper, testament, torch, wagon, whoremonger, worshipper); owner ruling for this pass was **whitelist and drop from the blacklist**, so `49_build_blacklist.py` gained a `tokenlist-manual` exclusion (deliberately scoped to that tag — the 132 older whitelist/blacklist overlaps are untouched, pending their own ruling) and was re-run with `--allow-shrink`: **2,693 → 2,667 words**, exactly those 26 removed and nothing else. The applied verse changes themselves stand; no text was altered (`db/mandela.db` untouched), so no republish was required. `scripts/82_full_token_list.py --allow-shrink` regenerated afterwards: **2,255 → 1,164 groups** (prior report archived to `references/removed_words/pre_triage_backups/token_list_full_pre_shrink_2255rows.md`); no whitelisted word survives in it. Per the owner's follow-up directive *"these lists are too large… generate the smaller list unless asked otherwise"*, script 82's **default output is now the condensed shape** — `# | word | count | forms`, one row per inflection group, **no verse-reference column** (181 KB → 57 KB); `--full` emits the wide report that carries references for groups of ≤8 uses, and switching a wide report to the condensed shape archives it first (`token_list_full_with_refs_*rows.md`). One group is above the old 88 ceiling — **`full` (264: full ×260, fuller ×1, fuller's ×3)**, which the archived report did not list, so script 83's ">88 uses" sweep never saw it and it was never whitelisted; re-checked against the pre-pass whitelist, it would have been listed then too, i.e. the 2,255-row report was stale relative to later text changes (790,499 → 790,485 words). Flagged for an owner ruling — it is the only group left that the "extremely common words" rule would have covered.
- [ ] **AWAITING OWNER RULINGS (generated 2026-07-29)** — **King James triage of the whole remaining token list.** `scripts/93_token_triage_batches.py` split every unprotected inflection group out of `references/word_lists/token_list_full.md` into agent batches (each group with its forms, count, and up to 3 occurrence verses from the *restored* text); parallel king-james agents ruled **KEEP / WHITELIST / REPLACE** on every one; `scripts/94_merge_token_triage.py` consolidated them (it refuses to overwrite a summary once an `OWNER RULING:` line is filled in, and cross-checks that no group went unruled).
  - **Round 1** (6 batches, 1,164 groups, `references/word_lists/token_triage_summary.md`): 1,099 KEEP, 58 WHITELIST, 7 REPLACE. Suggestions were given only for the REPLACE words.
  - **Round 2** (10 batches, 1,163 groups after dragon was re-whitelisted, `token_triage_summary_r2.md`, `--tag r2` so round 1's artifacts stand): **1,053 KEEP, 100 WHITELIST, 7 REPLACE** (+3 compound verdicts listed under "Unparsed entries"). Re-run under the owner's revised protocol — **every** group carries the three mandatory slots (`whitelist` / `witness` / `own`) whatever the verdict. Whitelist slot filled with a real candidate **84%** of the time (186 "none fits", each naming its nearest failing candidate); the witness slot found a **period** reading in **every** group — the modern-English fallback was never needed.
  - **REPLACE candidates, union of both rounds (11)**: silk (Proverbs 31:22 only), steel (bow of *bronze* in WLC), circumsized (Hebrews 10:20), damage (Ezra 4:13's literal `damage/hurt` placeholder), lewdness (Job 31:11 "an lewdness wickedness"), hem (fringes/tzitzit at Numbers 15:38, Deuteronomy 22:12), butler (**collides with rounds 1–2's own `cupbearer → butler`**), yarn, drams, tongs, transformed (transfigured at Matthew 17:2, Mark 9:2).
  - **Text defects surfaced incidentally, verified against the DB** (not word-choice questions): Genesis 1:26 "be more be more" (rows 3600 + 12016), Ezra 4:13 "damage/hurt" (row 3475 — the only `word/word` slash in the text), Joshua 9:4 "wine wineskins" (row 3065 left the modifier standing).
  - **Audit of the agent's failures (2026-07-29)** — `scripts/96_none_fits_rescue.py` → `references/word_lists/none_fits_rescue.md`. Of round 2's whitelist slots, 70 contain the phrase "none fits"; **31 are true failures** (no candidate named at all). The script tests each by corpus-wide alignment — every verse where the failed word occurs, against nine translations, keeping only whitelisted words a witness reads *instead of* ours — and **rescued 15 of the 31**. Verified counterexample: **pollute → profane** (both `profane` and `profaned` are whitelisted and the text already uses them 49×; Darby/ASV read "profane" in the same clause at Exodus 20:25, Leviticus 19:29, Numbers 18:32), plus **slaughter → smiting** (22 verses in YLT), **secret → hidden/mystery/concealed**, **dealt → acted**. So "no whitelisted word carries this sense" is not reliable as stated — it needs mechanical verification before it is trusted.
  - **Caveats recorded**: batch 5's frequency figures are estimates (marked `~`) after a connection drop killed its bulk query; multi-word suggestions report the frequency of their first word only; Geneva 1599's **Daniel 4 versification runs several verses ahead** of the KJV numbering (same class as the known Psalms offset), so earlier Daniel 4 witness comparisons in this project may have compared the wrong verses.
