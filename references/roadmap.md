...
- [ ] **AWAITING OWNER RULING (generated 2026-07-26)** — Rare-word **round 7** (100 rarest lemmas over the current output, now including round 6, the chief/head review and the pronoun/word-swap pass): `scripts/77_round7_rare_words.py` built `references/rare_word_round7_review.md` (names/whitelist/proper nouns excluded per owner directive; Geneva 1599 + Oxford 1769 readings per occurrence per the Rare-Word Review List Protocol), king-james agent proposals merged in by `scripts/78_merge_kj_round7.py` (100/100 filled). Agent findings needing an owner call: **circumsized** (Hebrews 10:20) reads as a meaning-inversion — Geneva/Oxford both have "consecrated"; **splenter** (Matthew 7:3-5) inverts the mote/beam size-contrast, both witnesses read "beam"; and six entries (lade, lifetime, likeminded, meant, naughtiness, outstretched, palmerworm) carry thee/thou and verb-agreement leaks left by the pronoun-restoration pass, proposed as grammatical repairs rather than word swaps
