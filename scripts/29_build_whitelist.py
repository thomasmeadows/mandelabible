...
A third source existed 2026-07-20 → 2026-07-27: "orphaned" round-1
`rare_word_replacements.md` rows carrying the internal note "no safe
one-word swap found". Removed per owner directive — those rows were never
owner-reviewed and never changed the text, so they do not belong on a
protection list. They stay off the blacklist regardless:
scripts/49_build_blacklist.py excludes that flag on its own.
...
def main():
    ...
    reviewed_words = sorted(whitelist)
