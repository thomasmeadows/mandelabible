#!/usr/bin/env python3
"""67_round6_rare_words.py — Round 6 rare-word review (owner directive
2026-07-22): recompute the rarest words over the CURRENT output (base KJV + all
approved restorations, now including round 5 — 91 owner-ruled rare-word
restorations, flaw_type round5_review), collapse inflections per Early Modern
...
Report only (no DB writes). Mirrors the round-5 builder (scripts/62); output:
references/rounds/round6/rare_word_round6_review.md. **src** flags base-KJV vs restoration-
introduced.
...
OUT = ROOT / "references" / "rounds" / "round6" / "rare_word_round6_review.md"
...
def main():
    ...
    geneva = load_geneva()
    L = ["# Round 6 — Rare Word Review (rarest first)", "",
         "*The 100 rarest lemmas over the current output (base KJV + all approved "
         "restorations through the round-5 apply). Inflections collapsed (EModE); "
