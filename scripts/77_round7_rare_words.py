#!/usr/bin/env python3
"""77_round7_rare_words.py — Round 7 rare-word review (owner directive
2026-07-26): recompute the rarest words over the CURRENT output (base KJV +
ALL approved restorations, now including round 6, the chief/head review, and
the pronoun/word-swap pass), collapse inflections per Early Modern English,
and list the 100 rarest lemmas.
...
Report only (no DB writes). Mirrors the round-6 builder (scripts/67); output:
references/rounds/round7/rare_word_round7_review.md. **src** flags base-KJV vs restoration-
introduced.
...
OUT = ROOT / "references" / "rounds" / "round7" / "rare_word_round7_review.md"
...
def main():
    ...
    geneva = load_geneva()
    L = ["# Round 7 — Rare Word Review (rarest first)", "",
         "*The 100 rarest lemmas over the current output (base KJV + all approved "
         "restorations through round 6, the chief/head review, and the "
         "pronoun/word-swap pass). Inflections collapsed (EModE); whitelisted "
         "words, names, places, and always-capitalized proper nouns skipped. "
         "For each word: its form counts; every verse it appears in with the "
         "current text plus the Geneva 1599 and Standard Oxford Edition (1769 "
